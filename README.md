...
本项目基于 2026 年 Ultralytics 最新发布的 YOLO26 目标检测算法，设计并实现了一套完整的车流量检测系统。系统串联了**数据集准备 → 模型训练 → ONNX 导出 → 车辆检测与跟踪 → 车流量统计与车速估算 → 桌面端应用打包**全流程。最终交付物包括可运行的 Python 源代码、Windows 桌面端可执行程序（exe）。
